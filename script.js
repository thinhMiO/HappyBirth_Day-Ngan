...
function createHintText() {
  ...
  context.globalAlpha = 1.0; // Đảm bảo không bị trong suốt

  // Vẽ chữ vào chính giữa canvas
  context.fillText(text, canvasSize / 2, canvasSize / 2);

  // Tạo texture và vật liệu
  ...
  const textMaterial = new THREE.MeshBasicMaterial({
    ...
    transparent: true,
    side: THREE.DoubleSide,
    depthTest: false, // Đảm bảo không bị vật thể khác che
  });

  // Plane đủ lớn để chữ không bị co lại
  ...
  hintText.renderOrder = 999; // Đảm bảo vẽ trên cùng
  scene.add(hintText);
}
